...
def Haffine_from_points(fp, tp):
    ...
    tmp2 = numpy.concatenate(
        (numpy.dot(C, numpy.linalg.pinv(B)), numpy.zeros((2, 1))), axis=1
    )
...
def H_from_ransac(fp, tp, model, maxiter=1000, match_threshold=10):
    ...
    H, ransac_data = ransac.ransac(
        data.T, model, 4, maxiter, match_threshold, 10, return_all=True
    )
...
def Haffine_from_ransac(fp, tp, model, maxiter=1000, match_threshold=10):
    ...
    data = numpy.vstack((fp, tp))
    H, ransac_data = ransac.ransac(
        data.T, model, 3, maxiter, match_threshold, 7, return_all=True
    )
    return H, ransac_data["inliers"]
